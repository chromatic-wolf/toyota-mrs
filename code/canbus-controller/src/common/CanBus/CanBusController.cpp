...
{
    _canData->canFrame->can_id = canID;      // CAN id as 0x036
    _canData->canFrame->can_dlc = frameSize; // CAN data length as 8
    _canData->canFrame->data[0] = data[0];   // first byte is the button state
    _canData->canFrame->data[1] = data[1];
    _canData->canFrame->data[2] = data[2];
    _canData->canFrame->data[3] = data[3];
    _canData->canFrame->data[4] = data[4];
    _canData->canFrame->data[5] = data[5];
    _canData->canFrame->data[6] = data[6];
    _canData->canFrame->data[7] = data[7];
